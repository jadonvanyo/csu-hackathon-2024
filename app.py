...
from helpers import api_key_validation, apology, decrypt_key, encrypt_key, get_differences, get_fernet_instance, get_imp_resp, get_tailored_question_full, get_tailored_question_partial, get_tailored_resume, login_required, usd
...
@app.route("/tailored_question", methods=["GET", "POST"])
@login_required
def tailored_question():
    ...
    if request.method == "POST":
        ...
        if request.form.get("coverletter") == "full":
            ...
            tailored_question = get_tailored_question_full(
                decrypt_key(encrypted_api_key, get_fernet_instance()), 
                request.form.get("company"), 
                request.form.get("jobdescription"), 
                request.form.get("jobtitle"), 
        ...
        # Else, assume the user wants a partial cover letter
            ...
            tailored_question = get_tailored_question_partial(
                decrypt_key(encrypted_api_key, get_fernet_instance()), 
                request.form.get("company"), 
                request.form.get("jobdescription"), 
                request.form.get("jobtitle"), 
        ...
        # If not past the 5 document limit, INSERT the data into history
            ...
            db.execute(
                ...
                request.form.get("jobtitle"), tailored_question, datetime.datetime.now()
            )
        ...
        return jsonify({
            'status': 'success',
            'message': 'Resume processed successfully',
            'tailored_question': render_template_string(markdown.markdown(tailored_question)),
        })
    ...
    # User reached route via GET (as by clicking a link or via redirect)
        ...
        return render_template("tailored_question.html", resume=resume)